	...
	/* Init MPI */
        ...
        //each non-MASTER task receive his portion of array
         ...
         for (dest=0; dest<numtasks; dest++) {
                ...
                MPI_Send(&offset,1, MPI_INT, next, tag1, MPI_COMM_WORLD);

                MPI_Send(&(dataSet.dataPoints[offset*D]), chunksize*D, MPI_DOUBLE,next,DATA_TAG,MPI_COMM_WORLD);

                //MPI_Send(&(dataSet.label[offset]),chunksize, MPI_INT,dest,LABEL_TAG,MPI_COMM_WORLD);

                MPI_Send(&(dataSet.index[offset]), chunksize, MPI_INT,next,INDEX_TAG,MPI_COMM_WORLD);
				

                MPI_Recv(&offset,1, MPI_INT, prev, tag1, MPI_COMM_WORLD, &status);

                MPI_Recv(&dataSet.dataPoints[offset*D], chunksize*D, MPI_DOUBLE, prev, DATA_TAG, MPI_COMM_WORLD, &status);
                
                //MPI_Recv(&dataSet.label[offset], chunksize, MPI_INT, source, LABEL_TAG,MPI_COMM_WORLD, &status);
                
                MPI_Recv(&dataSet.index[offset], chunksize, MPI_INT, prev, INDEX_TAG,MPI_COMM_WORLD, &status);
